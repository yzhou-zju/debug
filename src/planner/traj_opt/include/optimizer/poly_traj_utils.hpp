...
namespace poly_traj
{
    ...
    typedef Eigen::Matrix<double, 3, 6> CoefficientMat;//[a5 a4 a3 a2 a1 a0]对应[t^5,t^4,t^3,t^2,t^1,1]从高系数到低次系数
    ...
    public:
        ...
        inline Eigen::MatrixXd getPositions() const
        {
            ...
            for (int i = 0; i < N; i++)
            {
                positions.col(i) = pieces[i].getCoeffMat().col(5);//[a5 a4 a3 a2 a1 a0]对应[t^5,t^4,t^3,t^2,t^1,1]从高系数到低次系数
            }
    ...
    public:
        ...
        //仍然是固定采样分辨率K，每次得到计算这个，计算复杂度较高？？需要修改这部分或者说不调用这部分，等待仔细观看
