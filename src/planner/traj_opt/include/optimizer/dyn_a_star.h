...
private:
	...
	inline bool checkOccupancy(const Eigen::Vector3d &pos) { return (bool)grid_map_->isOccupancy(pos); }
	inline bool checkOccupancy_esdf(const Eigen::Vector3d &pos){
		double dist_map;
		grid_map_->getDistance(pos,dist_map);
		if (dist_map< resolution_ )
			return true;
		else
			return false;
	}
	...
	std::priority_queue<GridNodePtr, std::vector<GridNodePtr>, NodeComparator_Astar> openSet_;
...
public:
	...
	AStar(){grid_map_.reset(new GridMap);};
	~AStar();
	...
	bool astarSearchAndGetSimplePath(const double step_size, Eigen::Vector3d start_pt, 
									 Eigen::Vector3d end_pt, std::vector<Eigen::Vector3d> &final_simple_path, std::vector<Eigen::Vector3d> &a_star_path);
};
